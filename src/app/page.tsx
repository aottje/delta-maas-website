...
const PHONE_NUMBER = "+31 85 115 50 47";
const PHONE_TEL = "+31851155047";
...
export default function Home() {
  ...
  const [callPopupOpen, setCallPopupOpen] = useState(true);
  ...
  const CallPopup = () => (
    <div className="fixed bottom-6 right-5 z-[9997] flex flex-col items-end gap-3 md:bottom-8 md:right-8">
      {callPopupOpen && (
        <div className="w-[calc(100vw-40px)] max-w-[340px] rounded-2xl border border-white/15 bg-[#071426] p-5 text-white shadow-2xl ring-1 ring-black/10 md:w-[320px]">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#d6a751]">
                Direct contact
              </p>
              <p className="mt-3 text-xl font-extrabold">{PHONE_NUMBER}</p>
            </div>
            <button
              type="button"
              onClick={() => setCallPopupOpen(false)}
              className="rounded-full border border-white/15 px-3 py-1 text-sm font-bold text-white/80 transition hover:border-[#d6a751] hover:text-[#d6a751]"
              aria-label="Sluit bel popup"
            >
              ×
            </button>
          </div>
          ...
          <a
            href={`tel:${PHONE_TEL}`}
            className="mt-5 flex items-center justify-center rounded-xl bg-[#d6a751] px-5 py-4 text-sm font-extrabold uppercase tracking-wide text-white shadow-lg transition hover:bg-[#c9973f]"
          >
            Bel nu →
          </a>
        </div>
      ...
      <a
        href={`tel:${PHONE_TEL}`}
        className="flex h-16 w-16 items-center justify-center rounded-full bg-[#d6a751] text-white shadow-2xl transition hover:scale-105"
        aria-label="Bel Delta & Maas Vastgoed"
        onClick={() => setCallPopupOpen(false)}
      >
        <svg viewBox="0 0 24 24" className="h-7 w-7 fill-current" aria-hidden="true">
          <path d="M6.62 10.79c1.44 2.83 3.76 5.15 6.59 6.59l2.2-2.2c.3-.3.75-.39 1.14-.26 1.25.41 2.6.63 3.95.63.61 0 1.1.49 1.1 1.1v3.49c0 .61-.49 1.1-1.1 1.1C10.85 21.74 2.26 13.15 2.26 3.5c0-.61.49-1.1 1.1-1.1h3.5c.61 0 1.1.49 1.1 1.1 0 1.35.21 2.7.63 3.95.12.39.04.84-.27 1.14l-2.2 2.2Z" />
        </svg>
      </a>
    </div>
  );
