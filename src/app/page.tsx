...
const woningen: Woning[] = [
  { id: 1, img: "https://images.unsplash.com/photo-1545324418-cc1a3fa10c00?q=80&w=1200&auto=format&fit=crop" },
  { id: 2, img: "https://images.unsplash.com/photo-1600607687939-ce8a6c25118c?q=80&w=1200&auto=format&fit=crop" },
  { id: 3, img: "https://images.unsplash.com/photo-1518005020951-eccb494ad742?q=80&w=1200&auto=format&fit=crop" },
  { id: 4, img: "https://images.unsplash.com/photo-1600210492486-724fe5c67fb0?q=80&w=1200&auto=format&fit=crop" },
  { id: 5, img: "https://images.unsplash.com/photo-1600566753376-12c8ab7fb75b?q=80&w=1200&auto=format&fit=crop" },
  { id: 6, img: "https://images.unsplash.com/photo-1600585154526-990dced4db0d?q=80&w=1200&auto=format&fit=crop" },
  { id: 7, img: "https://images.unsplash.com/photo-1600607687920-4e2a09cf159d?q=80&w=1200&auto=format&fit=crop" },
  { id: 8, img: "https://images.unsplash.com/photo-1560518883-ce09059eeffa?q=80&w=1200&auto=format&fit=crop" },
];
...
export default function Home() {
  ...
  const handleContactSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    formData.append("access_key", "01241262-f2ac-4884-8f2b-dd675ed041d8");
    formData.append("subject", "Nieuw bericht via Delta & Maas website");
    formData.append("from_name", "Delta & Maas website");
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        alert("Verzenden is niet gelukt. Mail ons via info@deltaenmaas.nl.");
      }
    } catch {
      alert("Verzenden is niet gelukt. Mail ons via info@deltaenmaas.nl.");
    }
      ...
      <header className={`fixed left-0 top-0 z-[9999] w-full transition ${isWhite ? "bg-white shadow-lg" : "bg-transparent"}`}>
        <div className="mx-auto flex w-full max-w-[1760px] items-center justify-between px-5 py-4 md:px-8 lg:px-14 2xl:px-20">
          <button
            type="button"
            onClick={goHome}
            className="relative z-[10001] flex h-[76px] w-[150px] cursor-pointer items-center md:h-[100px] md:w-[220px] xl:h-[118px] xl:w-[260px]"
            aria-label="Ga naar home"
          >
            <img
              src="/logo.png"
              alt="Delta & Maas Vastgoed"
              className="pointer-events-none h-[90px] w-auto object-contain drop-shadow-xl md:h-[130px] xl:h-[175px]"
            />
          </button>
          ...
          <nav className={`hidden items-center text-[17px] font-extrabold uppercase tracking-[0.09em] ${isWhite ? "text-[#071426]" : "text-white"} xl:flex`}>
            <button type="button" onClick={() => goSection("diensten")} className="border-r border-current/20 px-9 py-4 hover:text-[#d6a751]">
              Diensten
            </button>
            <button type="button" onClick={goAanbod} className="border-r border-current/20 px-9 py-4 hover:text-[#d6a751]">
              Aanbod
            </button>
            <button type="button" onClick={() => goSection("over")} className="border-r border-current/20 px-9 py-4 hover:text-[#d6a751]">
              Over ons
            </button>
            <button type="button" onClick={() => goSection("contact")} className="px-9 py-4 hover:text-[#d6a751]">
              Contact
            </button>
          </nav>
          ...
          <div className="hidden items-center gap-4 lg:flex">
            <button
              ...
              className="rounded-md bg-[#d6a751] px-8 py-4 text-sm font-extrabold uppercase tracking-wide text-white shadow-lg transition hover:bg-[#c9973f]"
            >
              Neem contact op →
            </button>
          </div>
          ...
          <button
            ...
            className={`z-[10001] rounded-md border px-4 py-3 text-sm font-bold uppercase xl:hidden ${
              isWhite ? "border-[#071426]/20 text-[#071426]" : "border-white/40 text-white"
            }`}
          >
            Menu
          </button>
        ...
        {mobileOpen && (
          <div className="border-t border-slate-200 bg-white px-5 py-5 text-[#071426] shadow-xl xl:hidden">
            <div className="grid gap-3 text-left text-sm font-extrabold uppercase tracking-[0.12em]">
              <button type="button" onClick={() => goSection("diensten")} className="rounded-lg bg-slate-50 px-4 py-4 text-left">
                Diensten
              </button>
              <button type="button" onClick={goAanbod} className="rounded-lg bg-slate-50 px-4 py-4 text-left">
                Aanbod
              </button>
              <button type="button" onClick={() => goSection("over")} className="rounded-lg bg-slate-50 px-4 py-4 text-left">
                Over ons
              </button>
              <button type="button" onClick={() => goSection("contact")} className="rounded-lg bg-[#d6a751] px-4 py-4 text-left text-white">
                Contact
              </button>
            </div>
          </div>
  ...
  const WoningCard = ({ woning }: { woning: Woning }) => (
    <article className="group overflow-hidden rounded-xl bg-white text-[#071426] shadow-xl transition hover:-translate-y-1 hover:shadow-2xl">
      <div className="relative h-[230px] overflow-hidden">
        <img src={woning.img} alt="Vastgoed aanbod binnenkort beschikbaar" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
        <div className="absolute inset-0 bg-[#071426]/25" />
        <div className="absolute left-5 top-5 rounded-full bg-white/90 px-4 py-2 text-xs font-extrabold uppercase tracking-[0.16em] text-[#071426]">
          Binnenkort
        </div>
      </div>
      <div className="p-7">
        <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#d6a751]">Delta & Maas Vastgoed</p>
        <h3 className="mt-3 text-3xl font-extrabold">Coming soon</h3>
        <p className="mt-4 text-sm leading-7 text-slate-500">
          Binnenkort publiceren wij hier geselecteerde vastgoedkansen.
        </p>
      </div>
    </article>
  );
      ...
      <div className="mx-auto grid w-full max-w-[1760px] gap-14 lg:grid-cols-[0.75fr_1.25fr]">
        <div>
          ...
          <p className="mt-6 max-w-md text-lg leading-9 text-slate-300">
            Heeft u een vraag, wilt u samenwerken of meer informatie ontvangen? Wij staan voor u klaar.
          </p>
        ...
        <form onSubmit={handleContactSubmit} className="rounded-xl bg-white/10 p-6 shadow-2xl ring-1 ring-white/10 backdrop-blur md:p-8">
          ...
          <button type="submit" className="mt-5 rounded-md bg-[#d6a751] px-9 py-4 text-sm font-extrabold uppercase tracking-wide text-white transition hover:bg-[#c9973f]">
            Verstuur bericht →
          </button>
        </form>
      </div>
        ...
        <div className="grid gap-10 py-6 md:grid-cols-4">
          <div>
            ...
            <p className="mt-6 text-base leading-8 text-slate-300">
              Wij creëren vastgoedwaarde door visie, strategie en uitvoering te combineren.
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-base leading-8 text-slate-300">
              Projectbemiddeling<br />
              Vastgoedacquisitie<br />
              Ontwikkeling<br />
              Beleggingsobjecten
            </p>
          </div>
        ...
        <div className="fixed bottom-28 right-5 z-[9998] w-[calc(100%-40px)] max-w-[360px] overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-black/10 md:right-8">
          ...
          <div className="p-5">
            <p className="text-sm leading-6 text-slate-600">
              Heeft u een vraag over vastgoedacquisitie, projectbemiddeling of vastgoedkansen? Start direct een WhatsApp gesprek.
            </p>
      ...
      <main className="min-h-screen bg-[#f7f8fb] text-[#071426]">
        ...
        <section className="px-5 pb-24 pt-40 md:px-8 lg:px-14 lg:pt-48 2xl:px-20">
          <div className="mx-auto w-full max-w-[1760px]">
            ...
            <h1 className="font-serif text-4xl font-bold md:text-7xl">Binnenkort beschikbaar.</h1>
            <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-600">
              Wij werken aan een zorgvuldig geselecteerd aanbod. Nieuwe vastgoedkansen worden hier binnenkort gepubliceerd.
            </p>
            <div className="mt-10 flex flex-wrap gap-4">
              ...
              <button type="button" onClick={goHome} className="rounded-md border border-[#071426]/20 px-7 py-4 text-sm font-extrabold uppercase tracking-wide text-[#071426]">
                Naar homepage
              </button>
            </div>
            <div className="mt-12 grid gap-8 md:grid-cols-2 xl:grid-cols-4">
              {woningen.map((woning) => (
                <WoningCard key={woning.id} woning={woning} />
              ))}
            </div>
          </div>
        </section>
    ...
    <main className="min-h-screen bg-white text-[#071426]">
      ...
      <section id="diensten" className="bg-white px-5 py-20 md:px-8 lg:px-14 2xl:px-20">
        <div className="mx-auto grid w-full max-w-[1760px] gap-6 md:grid-cols-2 xl:grid-cols-4">
          {[
            ["◎", "Projectbemiddeling", "Wij verbinden vraag en aanbod in de vastgoedmarkt."],
            ["▥", "Vastgoedacquisitie", "Wij vinden de juiste locaties met maximale potentie."],
            ["↗", "Ontwikkeling", "Van concept tot realisatie: wij creëren waarde."],
            ["⚿", "Beleggingsobjecten", "Exclusieve vastgoedkansen met potentie."],
          ].map(([icon, title, text]) => (
      ...
      <section id="woningen" className="bg-[#071426] px-5 py-24 text-white md:px-8 lg:px-14 2xl:px-20">
        <div className="mx-auto w-full max-w-[1760px]">
          <div className="mb-12 flex flex-col justify-between gap-6 md:flex-row md:items-center">
            <div>
              <p className="mb-3 text-sm font-extrabold uppercase tracking-[0.25em] text-[#d6a751]">Aanbod</p>
              <h2 className="font-serif text-4xl leading-tight md:text-5xl">Vastgoedkansen binnenkort beschikbaar.</h2>
            </div>
      ...
      <section id="over" className="bg-[#f7f8fb] px-5 py-24 md:px-8 lg:px-14 2xl:px-20">
        <div className="mx-auto grid w-full max-w-[1760px] gap-16 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
          ...
          <div className="overflow-hidden rounded-xl shadow-lg">
            <img src="/rotterdam-skyline.jpg" alt="Rotterdam skyline" className="h-[360px] w-full object-cover md:h-[420px]" />
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#f7f8fb] px-5 py-20 md:px-8 lg:px-14 2xl:px-20">
        <div className="mx-auto max-w-[1760px]">
          ...
          <div className="mt-8 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            <iframe title="Rotterdam kaart" src="https://www.openstreetmap.org/export/embed.html?bbox=4.35%2C51.86%2C4.58%2C51.98&layer=mapnik" className="h-[480px] w-full md:h-[600px]" />
          </div>
        </div>
      </section>
